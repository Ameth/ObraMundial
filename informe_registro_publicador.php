...
//Años servicio
$SQL_AnioServicio = Seleccionar('uvw_tbl_PeriodosInformes', 'DISTINCT AnioServicio', "NumCong='" . $_SESSION['NumCong'] . "'", 'AnioServicio');
		...
		function DescargarZIP() {
			Swal.fire({
				...
				if (result.isConfirmed) {
					DescargarSAPDownload("filedownload.php", "zip=" + btoa('1') + "&file=" + btoa("s-21") + "&filtro=<?php echo base64_encode($Filtro); ?>&anio=<?php echo base64_encode($Periodo); ?>", true)
				}
			});
		}
	</script>
									...
									<div class="col-lg-3">
										<select name="TipoPublicador" class="form-control" id="TipoPublicador">
											...
											<?php while ($row_TipoPublicador = sqlsrv_fetch_array($SQL_TipoPublicador)) { ?>
												<option value="<?php echo $row_TipoPublicador['IDTipoPublicador']; ?>" <?php if ((isset($_GET['TipoPublicador'])) && (strcmp($row_TipoPublicador['IDTipoPublicador'], $_GET['TipoPublicador']) == 0)) {
																															echo "selected=\"selected\"";
																														} ?>><?php echo $row_TipoPublicador['TipoPublicador']; ?></option>
											<?php } ?>
										</select>
									</div>
									...
									<div class="col-lg-3">
										<select name="PrivServicio" class="form-control" id="PrivServicio">
											...
											<?php while ($row_PrivServicio = sqlsrv_fetch_array($SQL_PrivServicio)) { ?>
												<option value="<?php echo $row_PrivServicio['IDPrivServicio']; ?>" <?php if ((isset($_GET['PrivServicio'])) && (strcmp($row_PrivServicio['IDPrivServicio'], $_GET['PrivServicio']) == 0)) {
																														echo "selected=\"selected\"";
																													} ?>><?php echo $row_PrivServicio['PrivilegioServicio']; ?></option>
											<?php } ?>
										</select>
									</div>
								</div>
										...
										<tbody>
											<?php while ($row = sqlsrv_fetch_array($SQL)) { ?>
												<tr class="gradeX tooltip-demo">
													...
													<td><a href="rpt_informe_registro_publicador.php?id=<?php echo base64_encode($row['IDPublicador']); ?>&anio=<?php echo base64_encode($Periodo); ?>" class="btn btn-primary btn-sm" target="_blank"><i class="fa fa-download"></i> Descargar tarjeta S-21</a></td>
												</tr>
											<?php } ?>
										</tbody>
									</table>
