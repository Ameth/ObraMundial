			   ...
			   <form action="informes.php" method="post" class="form-horizontal" enctype="multipart/form-data" id="frmInformes">   
				...
				<table class="table table-striped table-hover dataTables-example" >
                    ...
                    <?php while($row=sqlsrv_fetch_array($SQL)){
						...
						?>
						 <tr>
							<td <?php if($row['IDEstado']==2){echo "class='text-danger'";}?>><?php echo $row['NombrePublicador'];?>
							...
							<td>
								...
								<label class="checkbox-inline i-checks">
									<input name="chkPrAux<?php echo $row['IDPublicador'];?>" type="checkbox" id="chkPrAux<?php echo $row['IDPublicador'];?>" value="1" <?php if($row_Informes['PrecAuxiliar']==1){echo "checked='checked'";}?>>
								</label>
								<?php }?>
							</td>
							<td><input name="Publicaciones[]" autocomplete="off" type="text" class="form-control text-right w-80" id="Publicaciones<?php echo $row['IDPublicador'];?>" maxlength="3" onKeyUp="revisaCadena(this);" onKeyPress="return justNumbers(event,this.value);" value="<?php echo $row_Informes['Publicaciones'];?>"></td>
							<td><input name="PresVideo[]" autocomplete="off" type="text" class="form-control text-right w-80" id="PresVideo<?php echo $row['IDPublicador'];?>" maxlength="3" onKeyUp="revisaCadena(this);" onKeyPress="return justNumbers(event,this.value);" value="<?php echo $row_Informes['Videos'];?>"></td>
							<td><input name="Horas[]" autocomplete="off" type="text" class="form-control text-right w-80" id="Horas<?php echo $row['IDPublicador'];?>" maxlength="3" onKeyUp="revisaCadena(this);" onKeyPress="return justNumbers(event,this.value);" value="<?php echo $row_Informes['Horas'];?>"></td>
							<td><input name="Revisitas[]" autocomplete="off" type="text" class="form-control text-right w-80" id="Revisitas<?php echo $row['IDPublicador'];?>" maxlength="3" onKeyUp="revisaCadena(this);" onKeyPress="return justNumbers(event,this.value);" value="<?php echo $row_Informes['Revisitas'];?>"></td>
							<td><input name="Cursos[]" autocomplete="off" type="text" class="form-control text-right w-80" id="Cursos<?php echo $row['IDPublicador'];?>" maxlength="3" onKeyUp="revisaCadena(this);" onKeyPress="return justNumbers(event,this.value);" value="<?php echo $row_Informes['Cursos'];?>"></td>
							<td><input name="Comentarios[]" autocomplete="off" type="text" class="form-control w-100" id="Comentarios<?php echo $row['IDPublicador'];?>" maxlength="50" value="<?php echo $row_Informes['Notas'];?>"></td>
						</tr>
					<?php }?>
					<?php if($Num==0){?>
